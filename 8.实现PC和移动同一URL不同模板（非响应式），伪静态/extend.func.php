    ...
    if(is_array($dtp->CTags))
    {
        foreach($dtp->CTags as $tida=>$ctag)
        {
			...
			if(($loadtype!='autofield' || ($loadtype=='autofield' && $ctag->GetAtt('autofield')==1)) && in_array($ctag->GetName(), $fieldsname) )
            {
				...
				$dede_addonfields .= '<ul class="a123">';
				switch ($type) {
					case 1:
						
						$dede_addonfields .= (preg_match("/&".$ctag->GetName()."=/is",$filterarr,$regm) ? '<a title="不限" href="'.str_replace("&".$ctag->GetName()."=".$fields_value,"",$filterarr).'">不限</a>' : '<span>不限</span>').'&nbsp;';
